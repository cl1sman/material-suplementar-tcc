# Material Suplementar — Análise Não Invasiva de Segurança

Anexos técnicos do TCC apresentado à Faculdade de Computação (FACOM) da  
Universidade Federal de Mato Grosso do Sul (UFMS).

**Autor:** Matheus Clisman Mariano da Silva  
**Orientador:** Prof. Dr. Carlos Alberto da Silva

---

## Sobre

Este repositório contém os artefatos técnicos produzidos durante a análise não invasiva
do sistema SICERT (certificados.ufms.br), como material suplementar ao artigo principal.

O artigo principal está disponível em: [repositorio.ufms.br](https://repositorio.ufms.br)

---

## Conteúdo

| Arquivo/Pasta | Descrição |
|---------------|-----------|
| `material_suplementar.pdf` | Anexos técnicos completos |
<!--
| `artigo_estudos_aplicados.pdf` | PDF do artigo principal |
| `material_suplementar.pdf` | Anexos técnicos completos |
| `coletados/` | Scripts de coleta e verificação de headers |
| `evidencias/` | Artefatos brutos da coleta com hashes SHA-256 |
| `img/` | Capturas das atividades formativas |
-->
---

## Trabalhos relacionados

| Autor | Título | Link |
|-------|--------|------|
| Fernandes e Silva (2025) | Vulnerabilidades em Apps Web — Leilões e EAD | [🔗](https://repositorio.ufms.br/handle/123456789/13210) |
| Vitorino e Silva (2025) | Vulnerabilidades WordPress em Instituições de Ensino | [🔗](https://repositorio.ufms.br/handle/123456789/12178) |
| Borges e Silva (2025) | Vulnerabilidades Web em Universidade Privada | [🔗](https://repositorio.ufms.br/retrieve/51c473d3-42a7-4230-b483-72ac6491c9a7/31967.pdf) |

---

> ⚠️ Toda a análise foi conduzida dentro de limites éticos rigorosos, sem exploração ativa,
> sem acesso a dados de terceiros e com valores sensíveis redigidos nas evidências.
